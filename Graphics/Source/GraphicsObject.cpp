...
void GraphicsObject::render() const
{
    ...
    g_OpenGLFuncs->glUniform4f(shaderProgram->diffuseColorLocation(), diffuseColor.r(), diffuseColor.g(), diffuseColor.b(), 1.0f);

    const Core::Color& specularColor = material.specularColor();
    g_OpenGLFuncs->glUniform4f(shaderProgram->specularColorLocation(), specularColor.r(), specularColor.g(), specularColor.b(), 1.0f);
    ...
    if (indices.size() > 0)
        g_OpenGLFuncs->glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(indices.size()), GL_UNSIGNED_INT, 0);
    else {
        const std::vector<float>& vertices = m_geometryObject.vertices();
        g_OpenGLFuncs->glDrawArrays(GL_TRIANGLES, 0, static_cast<GLsizei>(vertices.size() / Geometry::GeometryObject::kVerticesPerTriangle));
    }
}
...
bool GraphicsObject::probe(int x, int y) const
{
    ...
    return m_geometryObject.intersectBoundingBox(nearPoint, farPoint) && m_geometryObject.intersect(nearPoint, farPoint, nullptr);
}
...
void GraphicsObject::initialize()
{
    ...
    const std::vector<float>& vertices = m_geometryObject.vertices();
    g_OpenGLFuncs->glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(float), vertices.data(), GL_STATIC_DRAW);
    g_OpenGLFuncs->glVertexAttribPointer(kVertexLocation, Geometry::GeometryObject::kValuesPerVertex, GL_FLOAT, GL_FALSE, 0, 0);
    ...
    g_OpenGLFuncs->glBufferData(GL_ARRAY_BUFFER, normals.size() * sizeof(float), normals.data(), GL_STATIC_DRAW);
    g_OpenGLFuncs->glVertexAttribPointer(kNormalLocation, Geometry::GeometryObject::kValuesPerVertex, GL_FLOAT, GL_FALSE, 0, 0);
    ...
    if (indices.size() > 0) {
        ...
        g_OpenGLFuncs->glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(int), indices.data(), GL_STATIC_DRAW);
    }
...
Core::Vector3d GraphicsObject::glNearPoint(int x, int y) const
{
    ...
    gluUnProject(x, y, 0.0, modelViewMatrix.data(), projectionMatrix.data(),
        m_graphicsEnvironment.viewportTransform().data(), &nearPoint[0], &nearPoint[1], &nearPoint[2]);
...
Core::Vector3d GraphicsObject::glFarPoint(int x, int y) const
{
    ...
    gluUnProject(x, y, 1.0, modelViewMatrix.data(), projectionMatrix.data(),
        m_graphicsEnvironment.viewportTransform().data(), &farPoint[0], &farPoint[1], &farPoint[2]);
